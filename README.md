...
[docs/07-local-agent-interface.md](docs/07-local-agent-interface.md). Two of its ten items have
landed — the transport-neutral core, and progressive capability discovery — so a script can
already ask what this installation can simulate and submit a validated job without HTTP. The
local transports themselves (JSON-RPC over stdio, CLI, MCP) are still design.
