...
> **Status: M1, M2 and M2.5 done, M3 mostly.** Seven physics run end to end on real FEniCSx
> solves (potential flow, magnetostatics, steady and transient heat conduction, linear
> elasticity, axisymmetric electrostatics, and modal analysis) and three of them have demo
> pages, the four browser packages — SDK, geometry editor,
...
>
> **The wire protocol is at 1.16.** Six of the last ten minors came from adding physics
...
| **Nonlinear solves, and what their answer is worth** — a capability declares what its iteration converges to, what the reported `residual` measures and in what unit, and **what a caller receives when the tolerance is not reached**: `return` (an unconverged preview, which is what a relaxation sweep is for) or `fail`. Enforced, not documented — an adapter promising `fail` cannot hand back an unconverged field. Landed with temperature-dependent conduction, the first physics here whose operator depends on its own solution, checked against the Kirchhoff closed form | [`mock_heat_nonlinear.py`](server/fenixspoon/solvers/mock_heat_nonlinear.py), [`solvers/base.py`](server/fenixspoon/solvers/base.py) | ✅ protocol 1.16 |
| **Modal analysis** — natural frequencies and mode shapes as a third kind of question: no load, an answer that is a spectrum, and unrestrained structures supported with their three rigid-body modes counted rather than filtered. The frequencies are a curve whose abscissa is the mode number; the shapes are an index over artifacts | [`mock_modal.py`](server/fenixspoon/solvers/mock_modal.py), [`modes.py`](server/fenixspoon/modes.py), [ADR 0004](docs/adr/0004-a-mode-is-not-an-instant.md) | ✅ protocol 1.14 |
